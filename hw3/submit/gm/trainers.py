...
from loss import GANLossD, GANLossG, KLDivergenceLoss, ReconstructionLoss
...
class Trainer:
    ...
    def save_states(self, step, is_last=False):
        if not os.path.exists(f"./runs/{self.run_name}"):
            os.makedirs(f"./runs/{self.run_name}")
        file_name = (
            f"{self.run_name}_final.pt" if is_last else f"{self.run_name}_step{step}.pt"
        )
        torch.save(
            {
                "step": step,
                "model_state_dict": self.model.state_dict(),  # Save the unoptimized model
...
class VAETrainer(Trainer):
    def __init__(
        self, cfg: TrainingConfig, device, model: nn.Module, train_dataset, test_dataset
    ) -> None:
        super().__init__()
        self.cfg = cfg
        ...
        self.train_dataloader = DataLoader(
            train_dataset, batch_size=cfg.batch_size, num_workers=8, pin_memory=True
        )
    ...
    def fit(self):
        ...
        summary(
            self.model, input_data=torch.ones(1, *self.in_shape, device=self.device)
        )
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for x, _ in (pbar := tqdm(self.train_dataloader)):
                ...
                pbar.set_description(
                    f"Epoch {epoch}, batch loss {round(lossf, 3)}, kl loss {round(kl_loss.item(), 3)}, recon loss {round(recon_loss.item(), 3)}"
                )
