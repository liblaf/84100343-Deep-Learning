...
class MaskedMultiheadSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, attention_mask: Tensor = None):
        ...
        weighted_value = multi_head_self_attention(
            Q, K, V, self.mask[:, :, :T, :T], self.attention_dropout, attention_mask
        )
...
class FeedForwardNetworks(nn.Module):
    def __init__(self, cfg: TrainingConfig) -> None:
        super().__init__()
        self.fc1 = nn.Linear(
            cfg.embedding_dim, 4 * cfg.embedding_dim, bias=cfg.use_bias
        )
        self.fc2 = nn.Linear(
            4 * cfg.embedding_dim, cfg.embedding_dim, bias=cfg.use_bias
        )
...
class TransformerDecoderBlock(nn.Module):
    def __init__(self, cfg: TrainingConfig) -> None:
        super().__init__()
        self.cfg: TrainingConfig = cfg
        self.ln1 = nn.LayerNorm(cfg.embedding_dim, elementwise_affine=cfg.use_bias)
        self.mmsa = MaskedMultiheadSelfAttention(cfg)
        self.ln2 = nn.LayerNorm(cfg.embedding_dim, elementwise_affine=cfg.use_bias)
...
class TransformerDecoder(nn.Module):
    def __init__(self, cfg: TrainingConfig) -> None:
        ...
        self.postion_embedding_layer = nn.Embedding(cfg.block_size, cfg.embedding_dim)
        self.input_dropout = nn.Dropout(cfg.dropout_rate)
        self.decoder_blocks = nn.ModuleList(
            [TransformerDecoderBlock(cfg) for _ in range(cfg.n_layers)]
        )
        self.ln = nn.LayerNorm(cfg.embedding_dim, elementwise_affine=cfg.use_bias)
...
class GPT(nn.Module):
    ...
    def get_log_p(self, x: Tensor, attention_mask: Tensor = None):
        ...
        log_prob_output = torch.gather(
            log_prob_all_vocab, dim=2, index=y.unsqueeze(2)
        ).squeeze(2)
        return (log_prob_output * attention_mask).sum(dim=-1) / attention_mask.sum(
            dim=-1
        )

    @classmethod
    def from_pretrained(
        cls, cfg: TrainingConfig, ckpt: str = "./checkpoints/gpt2_pretrained.pt"
    ):
        model = GPT(cfg)
