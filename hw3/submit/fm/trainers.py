...
class SFTTrainer(Trainer):
    ...
    def fit(self):
        ...
        with trange(1, self.num_steps + 1, dynamic_ncols=True) as pbar:
            for step in pbar:
            ...
            with tqdm(self.train_dataloader) as pbar:
                for completions, attention_masks in pbar:
                    ...
                    pbar.set_description(
                        f"Epoch {epoch}, batch loss {round(lossf, 3)}, acc {round(accf, 3)}"
                    )
                    ...
                    if steps % self.eval_freq == 0:
                        with torch.no_grad():
                            self.model.eval()
                            losses, accs = [], []
                            for completions, attention_masks in tqdm(
                            ...
                            print(
                                f"Step: {steps + 1}, Test Loss: {total_loss}, Acc: {total_acc}"
                            )
