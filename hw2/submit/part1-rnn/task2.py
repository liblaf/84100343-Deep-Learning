...
type F1BI = Float[torch.Tensor, "1 batch_size input_size=3*32*32+3"]
...
class LSTM(nn.Module):
    ...
    def forward(
        ...
        input_gate: BH = torch.sigmoid(ai)
        forget_gate: BH = torch.sigmoid(af)
        output_gate: BH = torch.sigmoid(ao)
